...
#altanbariscomert@inovanettech.com

## (Tools-Email Kısmında gönderme yapacağınız maili ayarlayınız)
:local emailAddress "mailadresinizburayagelcek@gmail.com";


:local scriptMode "backup";
:local forceBackup false;

## Backup için şifre koyabilirsiniz
...
##------------------------------------------------------------------------------------------##
# AŞAĞIDAKİ KOMUTLARA NE YAPTIĞINIZI BİLMİYORSANIZ DOKUNMAYINIZ
##------------------------------------------------------------------------------------------##

#Script messages prefix
:local SMP "YedeklemeVeMail:"

:log info "\r\n$SMP script \"Mikrotik RouterOS otomatik yedekleme\" başladı.";

#EMAİL AYARLARI KONTROL EDİLİYOR
...
# Example:
# :put [$buGlobalFuncCreateBackups name="günlük-yedekleme"];
:global buGlobalFuncCreateBackups do={
	:log info ("$SMP Yedekleme başladı.");  
	...
	:log info ("$SMP Sistem Yedeği Oluşturuldu $backupFileSys");   
	...
	:log info ("$SMP Ayar Dosyası Dışa aktarıldı. $backupFileConfig");   
...
:local mailSubject   		"$SMP Device - $deviceIdentityName.";
:local mailBody 	 		"";

:local mailBodyDeviceInfo	"\r\n\r\nCihaz Bilgisi \r\nCihaz İsmi: $deviceIdentityName \r\nModel: $deviceRbModel \r\nSeri Numarası: $deviceRbSerialNumber \r\nRouterOS Versiyonu: $deviceOsVerInst ($[/system package update get channel]) $[/system resource get build-time] \r\nGüncel Routerboard FW: $deviceRbCurrentFw \r\nCihaz Açık Kalma Süresi: $[/system resource get uptime]";
...
:if ($updateStep = 1) do={
	:log info ("$SMP İlk aşama Başladı.");   
	...
	:log info ("$SMP Yeni bir backup oluşutulması için kontrol yapılıyor .");
	if ($forceBackup = true or $scriptMode = "backup" or $isOsNeedsToBeUpdated = true) do={
		:log info ("$SMP Backup oluşturuluyor.");
		if ($isOsNeedsToBeUpdated = true) do={
		...
		:set mailSubject	($mailSubject . " AYLIK BACKUP.");
		:set mailBody		($mailBody . "İlgili aya ait backup yapıldı ve eklere eklendi.");
...
:if ($isSendEmailRequired = true) do={
	:log info "$SMP mail gönderiliyor. Bu işlem biraz sürebilir.";
	:do {/tool e-mail send to=$emailAddress subject=$mailSubject body=$mailBody file=$mailAttachments;} on-error={
		:delay 5s;
		:log error "$SMP mail gönderilemedi ($[/tool e-mail get last-status]). Birazdan tekrar denenecek."

		:delay 5m;

		:do {/tool e-mail send to=$emailAddress subject=$mailSubject body=$mailBody file=$mailAttachments;} on-error={
			:delay 5s;
			:log error "$SMP mail gönderilemedi tekrar gönderilemedi ($[/tool e-mail get last-status])."
...
:log info "$SMP script \"Mikrotik RouterOS otomatik yedekleme\" yapıldı.\r\n";
